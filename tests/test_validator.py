...
from aws_croniter import validator


def _then_matches(regex: str, matches: List[str]):
    for match in matches:
        _check_match(regex, match)


def _check_match(regex: str, match: str):
    assert re.fullmatch(regex, match)


def _then_does_not_match(regex: str, matches: List[str]):
    for match in matches:
        _check_non_match(regex, match)


def _check_non_match(regex: str, match: str):
    assert not re.fullmatch(regex, match)


def test_slash_regex():
    minute_values = r"(0?[0-9]|[1-5][0-9])"  # [0]0-59
    given_regex = validator.AWSCronExpressionValidator.slash_regex(minute_values)
    given_valid_matches = ["*/10", "5/0", "1/10", "0/05", "0/15", "0/30", "55/1", "1-7/2"]
    ...
    _then_matches(given_regex, given_valid_matches)
    _then_does_not_match(given_regex, given_invalid_matches)


def test_list_slash_regex():
...
def test_month_regex():
    ...
    given_invalid_matches = ["0", "13", "600", "-1", "XZY", "JANUARY", "", "?", "L", "W", "#"]
    # given_invalid_matches = ["2/FEB"]

    _then_matches(given_regex, given_valid_matches)
    _then_does_not_match(given_regex, given_invalid_matches)


def test_day_of_week_regex():
...
def test_validator():
    ...
    invalid_expression_exceptions = [
        ...
        ("5/155 10 * * ? *", validator.AWSCronExpressionMinuteError),
    ...
    for valid_expression in valid_expressions:
        assert validator.AWSCronExpressionValidator.validate(valid_expression) == valid_expression
